package com.evolution.bootcamp.effects
...
object Effects {
  final class IO[A] private (private val run: () => A) {
    def map[B](f: A => B): IO[B] = IO[B](f(run()))
    ...
    def option: IO[Option[A]] = map(a => Option(a))
    def handleErrorWith[AA >: A](f: Throwable => IO[AA]): IO[AA] =
      Try(run()).recover[IO[AA]](PartialFunction.fromFunction(f)).get
    def redeem[B](recover: Throwable => B, map: A => B): IO[B] = new IO[B](() =>
      Try(map(run())).recover(PartialFunction.fromFunction(recover)).get
    )
    def redeemWith[B](recover: Throwable => IO[B], bind: A => IO[B]): IO[B] =
      Try(bind(run())).recover(PartialFunction.fromFunction(recover)).get
  ...
  object IO {
    ...
    def delay[A](body: => A): IO[A] = IO[A](body)
    def pure[A](a: A): IO[A] = IO[A](a)
    def fromEither[A](e: Either[Throwable, A]): IO[A] = e match {
      case Left(e)      => raiseError(e)
      case Right(value) => apply(value)
    }
    def fromOption[A](option: Option[A])(orElse: => Throwable): IO[A] = option.getOrElse[IO[A]](raiseError(orElse))
