package com.evolution.bootcamp.ds
...
object DataStructures {
  ...
  val totalVegetableCost: Int = {
    vegetableAmounts.map(p => vegetablePrices.getOrElse(p._1, 10) * p._2).sum
  }
  ...
  val totalVegetableWeights: Map[String, Int] = { // implement here
    for {
      (k, v) <- vegetableAmounts
      if vegetableWeights.contains(k)
    } yield (k, v * vegetableWeights(k))
  }
  ...
  def sortConsideringEqualValues[T](map: Map[T, Int]): List[(Set[T], Int)] = {
    map.foldLeft(List[(Set[T], Int)]())((acc, p) => {
      acc.find((e) => e._2 == p._2) match {
        case Some(value) => ((value._1 ++ Set(p._1)) -> p._2) :: acc.filterNot((e) => e._2 == p._2)
        case None => (Set(p._1) -> p._2) :: acc
      }
    }).sortBy(_._2)
  }
}
