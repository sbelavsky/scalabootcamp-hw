package com.evolution.bootcamp.async
...
object AsyncHomework extends App {
  ...
  val names = for {
    ...
    serverNames <- Future.sequence(links.map(fetchServerName))
  } yield serverNames.filterNot(_.isEmpty).map(_.get).distinct.sorted

  println(Await.result(names, 30.seconds))
