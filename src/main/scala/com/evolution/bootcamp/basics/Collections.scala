...
object LinkedList extends App {
  ...
  def findGap(l: List[Int]): Option[(Int, Int)] = {
    l match {
      case Nil => None
      case _ :: Nil => None
      case x :: y :: tail if x + 1 == y => findGap(tail)
      case x :: y :: _ => Some((x, y))
    }
  }
  ...
  def min(list: List[Int]): Option[Int] = {
    list.reduceLeftOption((acc, b) => if (b < acc) b else acc)
  }
  ...
  def count(s: String): List[(Char, Int)] = {
    @tailrec
    def countAcc(prev: (Char, Int), x: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      x match {
        case Nil => acc :+ prev
        case x :: tail if x == prev._1 => countAcc((prev._1, prev._2 + 1), tail, acc)
        case x :: tail if x != prev._1 => countAcc((x, 1), tail, acc :+ prev)
      }
    }
...
// leetcode profile: https://leetcode.com/sbelavsky/
// hometask:
